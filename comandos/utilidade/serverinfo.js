...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const adminsLength = guild.members.cache.filter((member) =>
      member.roles.cache.some(
        (role) => role.name == "Staff" || role.name == "Admin"
      )
    ).size;
    const botsLength = guild.members.cache.filter((member) =>
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**ID:** \`${serverID}\` | **Criador:** ${serverOwner}\n**Criado em:** ${createdAt}\n\n`
      )
      .addFields(fields)
      .setTimestamp();
